...
//#define  USE_LOCKS	
#define  USE_DL_PREFIX	// actived by ljb
//#define  USE_SPIN_LOCKS  1	// I guess this will active USE_SPIN_LOCKS if msversion correct.// actived by ljb
...
static FORCEINLINE void* win32mmap(size_t size) {
	...
	int pageCount = (size + gPageSize - 1) / gPageSize;
	if (!pageLiner->Acquire(pageCount, range))
	{
		return MFAIL;
	}
	void* ptr = VirtualAlloc((void*)(gBaseAddress + range.beg * gPageSize), pageCount * gPageSize, MEM_COMMIT, PAGE_READWRITE);
	return (ptr != 0) ? ptr : MFAIL;
}
...
#define fm gm
    ...
    erroraction:
// 	  if ( !( GMalloc->GetIsSkipFreeCheck() ||  GMalloc->GetIsExit() ) )//hack for QT
// 	  {
// 		  USAGE_ERROR_ACTION(fm, p);
// 	  }else
	  {
		  free( mem );//we call system free here.if crash call ljb
	  }
    postaction:
      POSTACTION(fm);
    }
